...
# Headless VM: stop the synapse/keyring client from probing a GUI secret store
# ("cannot open display"). Token auth via SYNAPSE_AUTH_TOKEN needs no keyring.
...
# Install the Synapse client. Inside a venv/conda this just works; on a PEP-668
# (externally-managed) system Python, fall back to --break-system-packages.
pip install --quiet --upgrade synapseclient \
  || pip install --quiet --upgrade --break-system-packages synapseclient
...
canon () {  # $1=challenge  $2=staging subdir  $3=canonical target path
  # Match subject folders BraTS-<CH>-<digits> only (NOT wrappers like BraTS-MEN-Train).
  local subj_parent
  subj_parent="$(dirname "$(find "staging/$2" -type d -name "BraTS-$1-[0-9]*" -print -quit)")"
  [ -d "$subj_parent" ] || { echo "!! no BraTS-$1-[0-9]* found under staging/$2"; return 1; }
  mkdir -p "$(dirname "$3")"
  ln -sfn "$(realpath "$subj_parent")" "$3"
  echo "  $3  ->  $subj_parent  ($(find "$subj_parent" -maxdepth 1 -type d -name "BraTS-$1-[0-9]*" | wc -l) subjects)"
}
...
V4_PARENT="$(dirname "$(find staging/BraTS-MEN-TRAIN-FIX-V4 -type d -name 'BraTS-MEN-[0-9]*' -print -quit)")"
MEN_TRAIN="dataset/ASNR-MICCAI-BraTS2023-MEN-Challenge-TrainingData/BraTS-MEN-Train"
n=0
for d in "$V4_PARENT"/BraTS-MEN-[0-9]*; do
  s="$(basename "$d")"
...
for CH in GLI MEN PED; do
  ...
  echo "  $CH train subjects: $(find -L "$P" -maxdepth 1 -type d -name "BraTS-$CH-[0-9]*" 2>/dev/null | wc -l)"
done
